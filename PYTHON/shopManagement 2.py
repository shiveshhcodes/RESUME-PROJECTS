...
# Function to add the product to the database
def prodtoTable(date, prodName, prodPrice, cursor, details, wn):
    db = mysql.connector.connect(user="root", passwd="root", host="localhost", database='Shop')
    my_cursor = db.cursor()
    
    # Inserting a product into the products table
    query = "INSERT INTO products (date, prodName, prodPrice) VALUES (%s, %s, %s)"
    values = (date, prodName, prodPrice)
...
# Example of calling the function with cursor, details, and wn
# cursor = db.cursor()
# details = (date, prodName, prodPrice)
# wn = some_window_object
# prodtoTable(date, prodName, prodPrice, cursor, details, wn)
